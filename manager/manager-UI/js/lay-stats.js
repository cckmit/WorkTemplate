...
layui.use(['form', 'table'], () => {
    ...
    $(() => {
        // 获取查询条件数据
        const queryData = form.val("query-data-form");
        // 将数据放入返回队列
    ...
    table.on('toolbar(statsTableFilter)', (obj) => {
        if ('return' === obj.event) {
            if (queryDataRecode.length > 1) {
                ...
                // 重置查询表单之后重新赋值
                $('#query-data-form')[0].reset();
                form.val('query-data-form', queryData);
    ...
    function getList(operator, queryData) {
        ...
        $.ajax({
            url: currentPage.module.server + currentPage.page + "/getList?" + param,
            type: 'GET',
            dataType: "json",
            async: false,
            success: (result) => {
