...
@WebServlet(urlPatterns = { "/xx8", "/pages/xx8" })
public class Countseat8Servlet  extends UIMoudleServlet{
	...
	public Object doOperator(HttpServletRequest request) {
		Vector<Mini_community_8> list = (Vector<Mini_community_8>) MiniGamebackDao.instance
				.findBySQL("select * from Mini_community_8", Mini_community_8.class);
		Map<Integer, Double> activeUpMap = Minitj_wx.getActiveUpMap();
		;
		asd(list);
		for (Mini_community_8 community_1 : list) {
		...
		asd(list);
		int i = 0;
		for (Mini_community_8 _community : list) {
			i++;
			String updateSql = "UPDATE mini_community_8 SET pos=" + i + " WHERE game_id = " + _community.game_id;
