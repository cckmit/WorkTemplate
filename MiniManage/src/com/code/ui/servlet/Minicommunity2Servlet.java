...
@WebServlet(urlPatterns = { "/minicommunity2", "/pages/minicommunity2" })
public class Minicommunity2Servlet extends UIMoudleServlet {
	...
	protected void newData(Object t) {
		try {
			Mini_community_2 model = (Mini_community_2) t;
	...
	public Class<?> getClassInfo() {
		return Mini_community_2.class;
	}

	protected Type getTypeof() {
		// TODO Auto-generated method stub
		return new TypeToken<Vector<Mini_community_2>>() {
		}.getType();
	}
	...
	public String doSpecJson(HttpServletResponse response) {
		if ("modifySort".equals(get("spec_type"))) {
			...
			// 更新至目标行位置
			String updateSql = "UPDATE mini_community_2 SET pos=" + targetGamePos + " WHERE game_id = " + sourceGameId;
			MiniGamebackDao.instance.execSQLCMDInfo(updateSql);

			// String point = get("point");
			String point = sourceGamePos > targetGamePos ? "top" : "bottom";
			if ("top".equals(point)) {
				updateSql = "UPDATE mini_community_2 SET pos=pos+1 " + "WHERE game_id <> " + sourceGameId
	...
	protected void deleteData(Object t) {
		...
		Mini_community_2 model = (Mini_community_2) t;
		String deleteSQL = "delete from Mini_community_2 where game_id=" + model.game_id;
		MiniGamebackDao.instance.execSQLCMDInfo(deleteSQL);
	}

	@SuppressWarnings("unchecked")
	public Vector<Mini_community_2> findData() {
		...
		Vector<Mini_community_2> list = (Vector<Mini_community_2>) MiniGamebackDao.instance.findBySQL(getSQL(),
				Mini_community_2.class);
		Vector<Mini_community_2> list1 = new Vector<Mini_community_2>();
		Map<Integer, Double> activeUpMap = Minitj_wx.getActiveUpMap();
		for (Mini_community_2 community_2 : list) {
			Vector<Mini_community_1> liststore1 = (Vector<Mini_community_1>) MiniGamebackDao.instance.findBySQL(
					"select *from mini_community_1 where game_id=" + community_2.game_id, Mini_community_1.class);
			Vector<Mini_community_3> liststore3 = (Vector<Mini_community_3>) MiniGamebackDao.instance.findBySQL(
					"select *from mini_community_3 where game_id=" + community_2.game_id, Mini_community_3.class);
			Vector<Mini_community_4> liststore4 = (Vector<Mini_community_4>) MiniGamebackDao.instance.findBySQL(
					"select *from mini_community_4 where game_id=" + community_2.game_id, Mini_community_4.class);
			Vector<Mini_community_5> liststore5 = (Vector<Mini_community_5>) MiniGamebackDao.instance.findBySQL(
					"select *from mini_community_5 where game_id=" + community_2.game_id, Mini_community_5.class);
			Vector<Mini_community_6> liststore6 = (Vector<Mini_community_6>) MiniGamebackDao.instance.findBySQL(
					"select *from mini_community_6 where game_id=" + community_2.game_id, Mini_community_6.class);
			Vector<Mini_community_7> liststore7 = (Vector<Mini_community_7>) MiniGamebackDao.instance.findBySQL(
					"select *from mini_community_7 where game_id=" + community_2.game_id, Mini_community_7.class);
			Vector<Mini_community_8> liststore8 = (Vector<Mini_community_8>) MiniGamebackDao.instance.findBySQL(
					"select *from mini_community_8 where game_id=" + community_2.game_id, Mini_community_8.class);
			Vector<Mini_community_9> liststore9 = (Vector<Mini_community_9>) MiniGamebackDao.instance.findBySQL(
					"select *from mini_community_9 where game_id=" + community_2.game_id, Mini_community_9.class);
			Vector<Mini_community_10> liststore10 = (Vector<Mini_community_10>) MiniGamebackDao.instance.findBySQL(
					"select *from mini_community_10 where game_id=" + community_2.game_id, Mini_community_10.class);
			if (community_2.pos == 0) {
				community_2.pos = 999;
			}
			Vector<Mini_game> listgameinfo = (Vector<Mini_game>) MiniGamebackDao.instance
					.findBySQL("select*from mini_game where game_id=" + community_2.game_id, Mini_game.class);
			for (Mini_game mini_game : listgameinfo) {
				community_2.game_name = mini_game.game_name;
				community_2.game_spread = mini_game.game_spread;
				community_2.game_sorttype_shop = mini_game.game_sorttype_shop;
				community_2.appid = mini_game.game_appid;
				String nameString = "";
				...
				if (!nameString.isEmpty()) {
					community_2.isblock_stores = nameString;
				}
				if (activeUpMap.get(community_2.game_id) == null) {
					community_2.activeUp = 0.0;
					continue;
				}
				community_2.activeUp = XWHMathTool.formatMath(activeUpMap.get(community_2.game_id), 3);
				// 计算活跃up
			}
			if (!isblock_stores.isEmpty() && !isblock_stores.equals("2")
					&& (community_2.isblock_stores == null || !community_2.isblock_stores.contains(isblock_stores))) {
				continue;
			}
			list1.add(community_2);
		}
		asd(list1);
		return list1;
	}
	...
	protected void otherPost(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			// 写入数据库
			String result = Mini_community_2.importFromExcel(vector);
