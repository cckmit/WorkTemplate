...
@WebServlet(urlPatterns = { "/minicommunity6", "/pages/minicommunity6" })
public class Minicommunity6Servlet extends UIMoudleServlet{
	...
	protected void newData(Object t) {
		try {
			Mini_community_6 model = (Mini_community_6) t;
	...
	public Class<?> getClassInfo() {
		return Mini_community_6.class;
	}

	protected Type getTypeof() {
		// TODO Auto-generated method stub
		return new TypeToken<Vector<Mini_community_6>>() {
		}.getType();
	}
	...
	protected void deleteData(Object t) {
		...
		String deleteSQL = "delete from Mini_community_6 where game_id=" + model.game_id;
	...
	public Vector<Mini_community_6> findData() {
		...
		Vector<Mini_community_6> list = (Vector<Mini_community_6>) MiniGamebackDao.instance.findBySQL(getSQL(),
				Mini_community_6.class);
		Vector<Mini_community_6> list3 = new Vector<Mini_community_6>();

		Map<Integer, Double> activeUpMap = Minitj_wx.getActiveUpMap();
		for (Mini_community_6 community_3 : list) {
	...
	public static void asd(Vector<Mini_community_6> list) {
		Collections.sort(list, new Comparator<Mini_community_6>() {
			/*
			 * int compare(Student o1, Student o2) 返回一个基本类型的整型， 返回负数表示：o1 小于o2，
			 * 返回0 表示：o1和o2相等， 返回正数表示：o1大于o2。
			 */
			public int compare(Mini_community_6 o1, Mini_community_6 o2) {
				if (o1.pos > o2.pos) {
					return 1;
				}
				if (o1.pos == o2.pos) {
	...
	public String doSpecJson(HttpServletResponse response) {
		if ("modifySort".equals(get("spec_type"))) {
			...
			// 更新至目标行位置
			String updateSql = "UPDATE mini_community_6 SET pos=" + targetGamePos + " WHERE game_id = " + sourceGameId;
			MiniGamebackDao.instance.execSQLCMDInfo(updateSql);

			// String point = get("point");
			String point = sourceGamePos > targetGamePos ? "top" : "bottom";
			if ("top".equals(point)) {
				updateSql = "UPDATE mini_community_6 SET pos=pos+1 " + "WHERE game_id <> " + sourceGameId
	...
	protected void otherPost(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			// 写入数据库
			String result = Mini_community_6.importFromExcel(vector);
