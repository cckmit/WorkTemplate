...
@WebServlet(urlPatterns = { "/minicommunity10", "/pages/minicommunity10" })
public class Minicommunity10Servlet extends UIMoudleServlet{
	...
	protected void newData(Object t) {
		try {
			Mini_community_10 model = (Mini_community_10) t;
	...
	public Class<?> getClassInfo() {
		return Mini_community_10.class;
	}

	protected Type getTypeof() {
		// TODO Auto-generated method stub
		return new TypeToken<Vector<Mini_community_10>>() {
		}.getType();
	}
	...
	protected void deleteData(Object t) {
		Mini_community_10 model = (Mini_community_10) t;
		String deleteSQL = "delete from Mini_community_10 where game_id=" + model.game_id;
	...
	public Vector<Mini_community_10> findData() {
		...
		Vector<Mini_community_10> list = (Vector<Mini_community_10>) MiniGamebackDao.instance.findBySQL(getSQL(),
				Mini_community_10.class);
		Vector<Mini_community_10> list3 = new Vector<Mini_community_10>();

		Map<Integer, Double> activeUpMap = Minitj_wx.getActiveUpMap();
		for (Mini_community_10 community_3 : list) {
	...
	public String doSpecJson(HttpServletResponse response) {
		if ("modifySort".equals(get("spec_type"))) {
			...
			// 更新至目标行位置
			String updateSql = "UPDATE mini_community_10 SET pos=" + targetGamePos + " WHERE game_id = " + sourceGameId;
			MiniGamebackDao.instance.execSQLCMDInfo(updateSql);

			// String point = get("point");
			String point = sourceGamePos > targetGamePos ? "top" : "bottom";
			if ("top".equals(point)) {
				updateSql = "UPDATE mini_community_10 SET pos=pos+1 " + "WHERE game_id <> " + sourceGameId
	...
	protected void otherPost(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			// 写入数据库
			String result = Mini_community_10.importFromExcel(vector);
