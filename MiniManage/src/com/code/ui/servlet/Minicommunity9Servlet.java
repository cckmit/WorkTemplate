...
@WebServlet(urlPatterns = { "/minicommunity9", "/pages/minicommunity9" })
public class Minicommunity9Servlet extends UIMoudleServlet{
	...
	protected void newData(Object t) {
		try {
			Mini_community_9 model = (Mini_community_9) t;
	...
	public Class<?> getClassInfo() {
		return Mini_community_9.class;
	}

	protected Type getTypeof() {
		// TODO Auto-generated method stub
		return new TypeToken<Vector<Mini_community_9>>() {
		}.getType();
	}
	...
	protected void deleteData(Object t) {
		...
		String deleteSQL = "delete from Mini_community_9 where game_id=" + model.game_id;
	...
	public Vector<Mini_community_9> findData() {
		...
		Vector<Mini_community_9> list = (Vector<Mini_community_9>) MiniGamebackDao.instance.findBySQL(getSQL(),
				Mini_community_9.class);
		Vector<Mini_community_9> list3 = new Vector<Mini_community_9>();

		Map<Integer, Double> activeUpMap = Minitj_wx.getActiveUpMap();
		for (Mini_community_9 community_3 : list) {
	...
	public String doSpecJson(HttpServletResponse response) {
		if ("modifySort".equals(get("spec_type"))) {
			...
			// 更新至目标行位置
			String updateSql = "UPDATE mini_community_9 SET pos=" + targetGamePos + " WHERE game_id = " + sourceGameId;
			MiniGamebackDao.instance.execSQLCMDInfo(updateSql);

			// String point = get("point");
			String point = sourceGamePos > targetGamePos ? "top" : "bottom";
			if ("top".equals(point)) {
				updateSql = "UPDATE mini_community_9 SET pos=pos+1 " + "WHERE game_id <> " + sourceGameId
	...
	protected void otherPost(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			// 写入数据库
			String result = Mini_community_9.importFromExcel(vector);
