...
@WebServlet(urlPatterns = { "/minicommunity5", "/pages/minicommunity5" })
public class Minicommunity5Servlet extends UIMoudleServlet{
	...
	public Class<?> getClassInfo() {
		return Mini_community_5.class;
	}

	protected Type getTypeof() {
		// TODO Auto-generated method stub
		return new TypeToken<Vector<Mini_community_5>>() {
		}.getType();
	}
	...
	protected void deleteData(Object t) {
		Mini_community_5 model = (Mini_community_5) t;
		String deleteSQL = "delete from Mini_community_5 where game_id=" + model.game_id;
	...
	public Vector<Mini_community_5> findData() {
		...
		Vector<Mini_community_5> list = (Vector<Mini_community_5>) MiniGamebackDao.instance.findBySQL(getSQL(),
				Mini_community_5.class);
		Vector<Mini_community_5> list3 = new Vector<Mini_community_5>();

		Map<Integer, Double> activeUpMap = Minitj_wx.getActiveUpMap();
		for (Mini_community_5 community_3 : list) {
	...
	public static void asd(Vector<Mini_community_5> list) {
		Collections.sort(list, new Comparator<Mini_community_5>() {
			/*
			 * int compare(Student o1, Student o2) 返回一个基本类型的整型， 返回负数表示：o1 小于o2，
			 * 返回0 表示：o1和o2相等， 返回正数表示：o1大于o2。
			 */
			public int compare(Mini_community_5 o1, Mini_community_5 o2) {
				if (o1.pos > o2.pos) {
					return 1;
				}
				if (o1.pos == o2.pos) {
	...
	public String doSpecJson(HttpServletResponse response) {
		if ("modifySort".equals(get("spec_type"))) {
			...
			// 更新至目标行位置
			String updateSql = "UPDATE mini_community_5 SET pos=" + targetGamePos + " WHERE game_id = " + sourceGameId;
			MiniGamebackDao.instance.execSQLCMDInfo(updateSql);

			// String point = get("point");
			String point = sourceGamePos > targetGamePos ? "top" : "bottom";
			if ("top".equals(point)) {
				updateSql = "UPDATE mini_community_5 SET pos=pos+1 " + "WHERE game_id <> " + sourceGameId
	...
	protected void otherPost(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			// 写入数据库
			String result = Mini_community_5.importFromExcel(vector);
