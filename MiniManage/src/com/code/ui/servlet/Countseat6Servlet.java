...
@WebServlet(urlPatterns = { "/xx6", "/pages/xx6" })
public class Countseat6Servlet extends UIMoudleServlet {
	...
	public Object doOperator(HttpServletRequest request) {
		Vector<Mini_community_6> list = (Vector<Mini_community_6>) MiniGamebackDao.instance
				.findBySQL("select * from Mini_community_6", Mini_community_6.class);
		Map<Integer, Double> activeUpMap = Minitj_wx.getActiveUpMap();
		;
		asd(list);
		for (Mini_community_6 community_1 : list) {
		...
		asd(list);
		int i = 0;
		for (Mini_community_6 _community : list) {
			i++;
			String updateSql = "UPDATE mini_community_6 SET pos=" + i + " WHERE game_id = " + _community.game_id;
