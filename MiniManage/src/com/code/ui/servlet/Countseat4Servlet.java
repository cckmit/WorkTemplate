...
@WebServlet(urlPatterns = { "/xx4", "/pages/xx4" })
public class Countseat4Servlet extends UIMoudleServlet {
	...
	public Object doOperator(HttpServletRequest request) {
		Vector<Mini_community_4> list = (Vector<Mini_community_4>) MiniGamebackDao.instance
				.findBySQL("select * from Mini_community_4", Mini_community_4.class);
		Map<Integer, Double> activeUpMap = Minitj_wx.getActiveUpMap();
		;
		asd(list);
		for (Mini_community_4 community_1 : list) {
		...
		asd(list);
		int i = 0;
		for (Mini_community_4 _community : list) {
			i++;
			String updateSql = "UPDATE mini_community_4 SET pos=" + i + " WHERE game_id = " + _community.game_id;
