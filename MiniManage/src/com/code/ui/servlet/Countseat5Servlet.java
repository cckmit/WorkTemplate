...
@WebServlet(urlPatterns = { "/xx5", "/pages/xx5" })
public class Countseat5Servlet extends UIMoudleServlet {
	...
	public Object doOperator(HttpServletRequest request) {
		Vector<Mini_community_5> list = (Vector<Mini_community_5>) MiniGamebackDao.instance
				.findBySQL("select * from Mini_community_5", Mini_community_5.class);
		Map<Integer, Double> activeUpMap = Minitj_wx.getActiveUpMap();
		;
		asd(list);
		for (Mini_community_5 community_1 : list) {
		...
		asd(list);
		int i = 0;
		for (Mini_community_5 _community : list) {
			i++;
			String updateSql = "UPDATE mini_community_5 SET pos=" + i + " WHERE game_id = " + _community.game_id;
