...
public class DBPoolConnection {
    private static Logger LOG = LoggerFactory.getLogger("stdoutsLog");
    ...
    public int[] execBatchSQL(Connection conn, BatchSQL batchSQL) {
        ...
        try {
            PreparedStatement rest = conn.prepareStatement(batchSQL.getSQL());
            conn.setAutoCommit(false);
            AtomicLong atomicLong = new AtomicLong();
            for (int i = 0; i < batchSQL.getLength(); i++) {
                batchSQL.addBatch(rest, i);
                rest.addBatch();
                if (atomicLong.incrementAndGet() % 2048 == 0) {
                    rest.executeBatch();
                }
            }
            executeBatch = rest.executeBatch();
            conn.commit();
            LOG.debug("execBatchSQL:" + batchSQL.getSQL() + ",length=" + atomicLong.get());
        } catch (Exception e) {
            LOG.error(Log4j.getExceptionInfo(e));
        } finally {
            if (conn != null) {
